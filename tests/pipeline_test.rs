...
//! Full transcription test requires Whisper model and is marked #[ignore].
//!
//! Run with: cargo test --test pipeline_test
//! Run transcription test: cargo test --test pipeline_test test_audio_to_text -- --ignored --nocapture
...
use whisperme::transcription::{TranscriptionThread, TranscriptionRequest};
...
fn test_process_jfk_audio() {
    ...
    println!("Processed to {} samples at {}Hz", audio_16k.len(), SAMPLE_RATE);
    
    // 11 seconds at 48kHz → ~11 seconds at 16kHz
    // Input: ~528000 samples, Output: ~176000 samples (minus warmup)
    assert!(audio_16k.len() > 170000, "Expected at least 170k samples, got {}", audio_16k.len());
...
/// Requires Whisper model - run with: cargo test --test pipeline_test test_audio_to_text -- --ignored --nocapture
#[test]
#[ignore]
fn test_audio_to_text_pipeline() {
    // Load test audio
    let audio_path = Path::new("tests/fixtures/jfk_48k.wav");
    assert!(audio_path.exists(), "Test audio file not found: {:?}", audio_path);
    ...
    // Load config and create transcription thread
    let config = Config::load();
    assert!(config.whisper.model_path.exists(), 
        "Whisper model not found: {:?}. Download a model first.", 
        config.whisper.model_path);
    
    println!("Loading Whisper model: {:?}", config.whisper.model_path);
